...
async function handleFiles(files) {
  ...
  if (extracted.length) {
    addMergedNote(extracted);
    setStatus(extracted.length > 1 ? `Merged ${extracted.length} lectures into one course pack.` : `Uploaded ${extracted[0].title}`);
  }

  $("#fileInput").value = "";
  saveWorkspace();
  renderAll();
  if (extracted.length) setView("dashboard");
}
...
function addMergedNote(items) {
  const title = items.length === 1 ? items[0].title : buildMergedTitle(items);
  const text = items.length === 1
    ? items[0].text
    : items.map((item, index) => `Lecture ${index + 1}: ${item.title}\n${item.text}`).join("\n\n---\n\n");
  addNote(title, text, items);
}

function addNote(title, text, sources = null) {
  ...
  const note = {
    ...
    sources: (sources || [{ title }]).map((item) => item.title),
    createdAt: new Date().toISOString(),
...
function cleanFileTitle(name) {
  return String(name).replace(/\.(pdf|txt|docx|pptx|png|jpe?g|webp|bmp|gif|md|csv)$/i, "").replace(/[_-]+/g, " ").trim() || "Lecture";
}

function buildMergedTitle(items) {
  const first = items[0]?.title || "Course";
  const compact = first.replace(/\b(lecture|lec|chapter|chap|week|class|part)\s*\d+\b/ig, "").trim();
  return `${compact || "Course"} - merged ${items.length} lectures`;
}
    ...
    <article class="note-item">
      <div class="note-title-row">
        <div>
          ...
          <div class="note-meta">${note.wordCount} words | ${note.sourceCount || 1} lecture${(note.sourceCount || 1) === 1 ? "" : "s"} | ${formatDate(note.createdAt)}</div>
        </div>
...
function startVoiceAsk() {
  ...
  const selectedLanguage = $("#voiceLangSelect")?.value || "auto";
  recognition.lang = selectedLanguage === "auto" ? (navigator.language || "en-US") : selectedLanguage;
  recognition.interimResults = false;
  recognition.continuous = false;
  recognition.onstart = () => {
    $("#voiceAskBtn").classList.add("listening");
    $("#voiceStatus").textContent = `Listening (${recognition.lang}) for your lecture question...`;
  };
...
function renderChat() {
  $("#chatLog").innerHTML = state.chat
    .map((message) => `<div class="chat-message ${message.role === "user" ? "user" : ""}" dir="auto">${escapeHtml(message.text)}</div>`)
    .join("");
...
function sentencesFrom(text) {
  ...
  const sentences = normalized.split(/(?<=[.!?؟۔।。])\s+/u).filter((sentence) => sentence.length > 20);
  if (sentences.length) return sentences;
  return normalized.match(/.{1,240}(?:\s|$)/gu)?.map((chunk) => chunk.trim()).filter(Boolean) || [];
}
